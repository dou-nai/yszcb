...
class GoodsController extends BaseController
{
    ...
    //卡号列表
    public function Card_list(Request $req,GoodsCard $goodsCard){
        $where=[];
        if($req->get("id"))
        {
            $goodsCard=$goodsCard->where("goods_id",$req->get("id"));
        }
        if($req->get("sn"))
        {
            $goodsCard=$goodsCard->where("sn",$req->get("sn"));
        }
        if($req->get("status"))
        {
            $goodsCard=$goodsCard->where("status",$req->get("status"));
        }

        $list=$goodsCard->with(['user'])->orderBy('id','desc')->paginate(20);
        return $this->success_msg('Success',$list);
    }
    //卡号核销
    public function write_off(Request $req,GoodsCard $goodsCard)
    {
        ...
        $g=$goodsCard->where("id",$req->get("id"))->update(['status'=>1,'pay_time'=>time(),'user_id'=>'0','order_id'=>0,'order_no'=>'']);
        return $this->success_msg('Success',$g);
    }

    // 添加商品
    public function add(Request $req,Goods $goods_model,GoodsCard $goodsCard,GoodsBrand $goods_brand_model,GoodsSpec $goods_spec_model,Config $config_model,GoodsClass $goods_class_model,StoreGoodsClass $store_goods_class_model){
    ...
    // 编辑商品
    public function edit(Request $req,Goods $goods_model,GoodsCard $goodsCard,GoodsBrand $goods_brand_model,GoodsSpec $goods_spec_model,Config $config_model,AttrSpec $attr_spec_model,GoodsClass $goods_class_model,FreightTemplate $freight_template_model,StoreGoodsClass $store_goods_class_model,$id){
        if(!$req->isMethod('post')){
            $data['goods_brand_list'] = $goods_brand_model->get();
            $data['store_goods_class'] = $store_goods_class_model->get();
    ...
    public function card_add(Request $req,GoodsCard $goodsCard)
    {
        $cards= $req->get("cards");
        $card_arr=explode(",",$cards);
        $gid=$req->get("goods_id");
        DB::transaction(function () use($gid,$card_arr) {
            foreach($card_arr as $card)
            {
                $card=$card.mt_rand(10,99);
                $card=$card.$this->crc16($card);
                GoodsCard::insertGetId(["goods_id"=>$gid,"sn"=>$card,"status"=>0]);
            }
        });
