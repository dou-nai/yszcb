...
class PayController extends BaseController
{
    ...
    public function pay(Request $req,Config $config_model,Order $order_model,Helper $helperObj){
        ...
        switch($payment_name){
            ...
            case 'wxpay_jsapi':
                // $config_biz = [
                //     'out_trade_no'     => 'wechatpay_uid_' . $user->id . '_' . strtotime(date("Y-m-d H:i:s")), // 订单号 php strtotime 为10位时间戳
                //     'total_fee'        => $total_amount * 100, // 订单金额，**单位：分**
                //     'attach'           => $user->id, // 额外信息，可自定义填写
                //     'body'             => $card->title, // 订单描述
                //     'spbill_create_ip' => $_SERVER['REMOTE_ADDR'], // 调用 API 服务器的 IP
                //     'openid'           => $user->wx_openid, // 用户 openid ID
                //     'trade_type'       => 'JSAPI',
                // ];
            ...
            case 'wxpay_mini':
                ...
                $this->wx_config['key'] = $db_config['key'];
                //$this->wx_config['notify_url'] = $db_config['notify_url'];
                $pay_order_info["openid"] = $user_info["open_id"];
