...
class PayMentController extends Controller
{
    ...
    public function pay_ok($order_id)
    {
        ...
        $res=Order::where("id", $order_id)->update(["pay_status"=>1, "pay_time"=>time(), "pay_type"=>"wx_mini"]);
        $order= Order::where("id", $order_id)->with('order_goods')->first();

        foreach ($order->order_goods as $key=> $item){

            $goods= Goods::where("id",$item->goods_id)->first();

            if($goods->is_virtual=="1") {
                DB::transaction(function () use($item,$order) {
                    $card = GoodsCard::where('status', 0)->where('goods_id',$item->goods_id)->orderBy("id","asc")->lockForUpdate()->first();
                    $card->order_no = $order->order_no;
                ...
                Order::where("id", $order_id)->update(["delivery_status"=>1, "delivery_time"=>time(),"is_hexiao"=>1]);
            }
        }
    ...
    protected function get_payment_config($payment_name){
        ...
        switch($payment_name){
            ...
            case "wxpay_mini":
                // 配置支付密钥
                $db_config = json_decode($config_info['wxpay_mini'], true);
                $this->wx_config['app_id'] = $db_config['appid'];
                $this->wx_config['mch_id'] = trim($db_config['mchid']);
                $this->wx_config['miniapp_id'] = $db_config['appid'];
                $this->wx_config['key'] = $db_config['key'];
