...
{
    // 商品详情
    public function get_goods_info(Request $req,Goods $goods_model,GoodsClass $goods_class_model,Store $store_model,StoreComment $store_comment_model,Groupbuy $groupbuy_model,GroupbuyUser $groupbuy_user_model,SeckillGoods $seckillGoods){
        ...
        $seckill=$seckillGoods->where('goods_id',$goods_id)->first();
        if($seckill)
        {
            $data['seckill_info']=Seckill::where("id",$seckill->sid)->first();
    ...
    public function search_goods(Request $req,Goods $goods_model,GoodsClass $goods_class_model){
        ...
        //商家商品
        if(!empty($req->user_id) && isset($req->user_id)){
            $where['user_id'] = $req->user_id;
        }
        $order = [];

        // 条件筛选选判断  0 默认 1 价格 2 销量 3 评论
        ...
        $store_id = 0;
        if(!empty($req->store_id) && isset($req->store_id)){
        ...
        if(!empty($req->class_id) && isset($req->class_id)){
            $class_id=$req->class_id;
        }

        $pageSize=20;
        if(!empty($req->pageSize) && isset($req->pageSize)){
            $pageSize=$req->pageSize;
        }
        $data = $goods_class_model->searchGoods($class_id,$where,$order,$keywords,$pageSize,$store_id);
