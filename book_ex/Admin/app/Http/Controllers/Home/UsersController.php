...
class UsersController extends BaseController
{
    ...
    // 修改用户信息
    public function edit_user_info(Request $req,Users $user_model,SmsLog $sms_log_model,GoodsClass $goods_class_model){
        $user_info = auth()->user();
        if(empty($user_info)){
            return $this->error_msg('请先登录');
        }
        ...
        if(!$req->isMethod('post')){
            if(!empty($user_info['like_cate_ids'])){
                $like_cate_ids=explode(',',$user_info['like_cate_ids']);
                $user_info['tags']=$goods_class_model->whereIn('id',$like_cate_ids)->get();
            }else{
                $user_info['tags']='';
            }
        ...
        if(isset($req->like_cate_ids)){
            $data['like_cate_ids'] = $req->like_cate_ids;
        }
    ...
    public function get_fav_list(Request $req,Favorites $favorites_model){
        ...
        if(empty($type)){
            $favorites_model = $favorites_model->has('goods')->with(['goods'=>function($q){
                $q->with('class');
            }]);
        }else if($type==2){
            $favorites_model = $favorites_model->with(['class']);
        }else{
