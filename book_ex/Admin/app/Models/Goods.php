...
class Goods extends Model
{
    ...
    public function class(){
        return $this->hasOne('App\Models\GoodsClass','id','cid');
    }
    ...
    public function getGoodsInfoById($id){
        ...
        $goods_info = $this->with('spec')->find($id);
        if(empty($goods_info)){
            return [];
        }
        ...
        if( $seckill_model->where('enable', 1)->exists()) {

            $seckill_goods_model = new SeckillGoods();
            $is_seckill_goods = $seckill_goods_model->where('sid', $seckill_model['id'])->where('goods_id', $id)->exists();

            $goods_info['is_seckill'] = 0;
            if ($is_seckill_goods) {
                $goods_info['is_seckill'] = 1;
                $seckill_info = $seckill_model->where('enable', 1)->first()->toArray();
