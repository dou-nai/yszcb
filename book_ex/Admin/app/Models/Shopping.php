...
class Shopping extends Model
{
    ...
    public function add_cart($data){
        ...
        if(!empty($cart_info)){
            ...
            $rs = $this->where('id',$cart_info['id'])->increment('goods_num',$data['goods_num']);  // 购物商品加上
            if($rs){
                ...
                return ['status'=>false,'msg'=>'购物商品添加错误'];
            }
        ...
        return ['status'=>true,'msg'=>'加入购物商品成功'];

    }

    // 编辑购物商品商品信息 false 减 true  加  $data['id']  $data['goods_num'] $data['type']
    public function change_cart($data,$type=false){
        $is_change_num = $data['type']??false; // 是否直接改变数量


        $cart_info = $this->where('id',$data['id'])->first(); // 获取这个购物商品商品的信息

        if(empty($cart_info)){
            return ['status'=>false,'msg'=>'不存在该购物商品商品'];
        }
    ...
    public function del_cart($ids){
        $user_info = auth()->user();
        $cart_list = $this->where('user_id',$user_info['id'])->whereIn('id',$ids)->get(); // 获取到购物商品列表
        // 获取属性规格模型
        $goods_spec_model = new GoodsSpec();
        $goods_model = new Goods();
