...
class Order extends Model
{
    ...
    public function create_order($data){
        ...
        if($is_cart){
            $store_list = $this->get_befor_order($data['data'],true,$data['is_groupbuy'], $is_scene);
        }else{
            $store_list = $this->get_befor_order($data['data'],false,$data['is_groupbuy'], $is_scene);
        }
        if(!$data['is_groupbuy']){
            unset($data['is_groupbuy']);
        }
        // 随机数生成  订单号
        $make_rand = make_rand();
        ...
        // if($is_shopping && !empty($data['sh_data'])){
        //   $address_info = $data['sh_data'];
        // }
        ...
        try{
            ...
            foreach($store_list['data'] as $v){
                ...
                foreach($v as $vo){
                    ...
                    // 判断中间是否有快递模版  同一个出版社订单 优先模版  取最多钱的
                    if($vo['freight_id']>0){
            ...
            // 删除购物商品 数据
            if($is_scene){
                $shopping_model = new Shopping();
                $shopping_model->whereIn('id',$store_list['cart_id'])->delete();
            }
    ...
    // 获取订单信息
    public function get_befor_order($data,$is_cart,$is_groupbuy=false,$is_scene=false){
        $goods_model = new Goods();
        ...
        foreach($store_arr as $k=>$v){
            ...
            foreach($arr[$k] as $key=>$vo){
                ...
                if(isset($v['buy_num'][$index])){
                    ...
                    $g= \App\Models\Goods::where("id",$vo['id'])->first();
                    if($g->goods_num<$vo['goods_num'])
                    {
                        $res_arr['data']=null;
                        $res_arr['msg']="库存数量不足!";
                        return $res_arr;
    ...
    public function order_goods(){
     ...
     // 关联到commission_order表
     public function commission_order(){
    	return $this->hasMany('App\Models\CommissionOrder','orderid','id')->with(['xd_user','fl_user'])->orderBy('cengji','asc');
    }
