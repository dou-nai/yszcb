...
class GoodsClass extends Model
{
    ...
    public function getGoodsListByClassId($ids,$where=[],$order=[],$take=0){
        ...
        $goods_list = $goods_model->with(['spec_once','class'])->where($this->params)->whereIn('cid',$ids)->get()->toArray();
    ...
    // 搜索产品
    public function searchGoods($class_id,$where=[],$order=[],$keywords="",$page=30,$store_id=0){
        ...
        // select 子查询评论数量
        $goods_model = $goods_model->with('class')->select(DB::raw('(select count(*) from '.$prefix.'store_comment as B where '.$prefix.'goods.id = B.goods_id) as comment_count,goods.*'));
        ...
        if(!empty($store_id)){
            ...
            $goods_model = $goods_model->where('store_id',$store_id);
        }
        ...
        $goods_res = $goods_model->where('goods_status',1)->orderBy('is_index','desc')->orderBy('id','desc')->with('spec_once')->paginate($page)->toArray();
