        ...
        <div class="admin_main_block">
            <div class="admin_main_block_top">
                <div class="admin_main_block_left">
                    <div><router-link to="/Admin/goods/chose_class"><el-button type="primary" icon="el-icon-plus">添加</el-button></router-link></div>

                    <div><el-input v-model="where.goods_name" placeholder="图书名"></el-input></div>
                     <div style="width:160px;">
                         <el-select v-model="where.goods_status" placeholder="选择类型">
            ...
            <div class="admin_table_main">
                <el-table :data="list" @selection-change="handleSelectionChange" >
                    ...
                    <el-table-column label="图书名称" width="400px">
                    ...
                    <el-table-column label="图书编号">
                        <template slot-scope="scope">
                           <dd class="table_dl_dd_all">{{ scope.row.goods_no }}</dd>
                        </template>
                    </el-table-column>
                    <el-table-column label="作者">
                        <template slot-scope="scope">
                           <dd class="table_dl_dd_all">{{ scope.row.author }}</dd>
                        </template>
                    </el-table-column>
                    <el-table-column label="版别">
                        <template slot-scope="scope">
                           <dd class="table_dl_dd_all">{{ scope.row.edition }}</dd>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="280px">
                        <template slot-scope="scope">
                           <el-button icon="el-icon-edit" @click="$router.push('/Admin/goods/edit/'+scope.row.id)"></el-button>
                            <el-button type="danger" size="mini" icon="el-icon-delete" @click="del(scope.row.id)"></el-button>

                            <el-button v-if="scope.row.is_virtual=='1'" type="danger" size="mini" icon="el-icon-s-grid" @click="$router.push('/Admin/goods/card/'+scope.row.id)"></el-button>
                        </template>
                    </el-table-column>
                </el-table>
...
export default {
    ...
    methods: {
        ...
        verify_click:function(id){
            this.$confirm('此操作将控制图书是否通过审核, 是否继续?', '提示', {
