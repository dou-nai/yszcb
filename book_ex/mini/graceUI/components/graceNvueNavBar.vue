<template>
	...
	:scroll-into-view="'tab-'+currentIndex" :show-scrollbar="false">
		...
		v-for="(item, index) in items" :key="index" @click="navchang" :data-index="index">
			...
			:style="{color:currentIndex == index ? activeColor:color, width:size+'rpx', lineHeight:lineHeight, fontSize:currentIndex == index ? activeFontSize : fontSize, textAlign:textAlign}">{{item}}</text>
